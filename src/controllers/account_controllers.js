...
/**
 * POST /coa/seed - Seed default COA
 * Query params: 
 *   - masjidId (optional) - Masjid ID untuk COA custom per masjid
 *   - general (optional, true/false) - Jika true, buat general/default COA (masjidId = null)
 */
exports.seedDefaultCOA = async (req, res) => {
  try {
    const { masjidId, general } = req.query;
    const userId = req.user.id;

    let targetMasjidId = null;
    
    // Jika parameter 'general=true', buat default COA tanpa masjidId (general)
    if (general === 'true' || general === true || general === '1') {
      targetMasjidId = null; // General/default COA untuk semua masjid
    } else {
      // Logic existing: ambil dari query atau dari user
    ...
    const result = await accountService.seedDefaultCOA();

    const message = targetMasjidId 
      ? `Default COA seeded successfully for masjid. ${result.count} accounts created.`
      : `General/default COA seeded successfully. ${result.count} accounts created.`;
    ...
    console.error("Error in seedDefaultCOA:", err);
    if (err instanceof CustomError) {
      return errorResponse(res, err.message, err.statusCode);
    }
    errorResponse(res, "Failed to seed default COA: " + err.message);
  }
};
