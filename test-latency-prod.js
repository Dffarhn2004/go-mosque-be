// test-latency-prod.js
// Script untuk test latency COA dan Laporan Keuangan di PRODUCTION
...
const BASE_URL = process.env.PROD_BASE_URL || 'https://goqu-be.vercel.app/api/v1';
const JWT_TOKEN = process.env.PROD_JWT_TOKEN || ''; // Set JWT token di .env atau hardcode untuk testing
const EMAIL = process.env.PROD_EMAIL || process.env.EMAIL || 'Hikmah@gmail.com'; // Email untuk auto-login
const PASSWORD = process.env.PROD_PASSWORD || process.env.PASSWORD || 'Hikmah123'; // Password untuk auto-login
const MASJID_ID = process.env.PROD_MASJID_ID || process.env.MASJID_ID || ''; // Set masjidId untuk testing
  ...
  try {
    ...
    if (response.data?.data?.token) {
      ...
      if (user.masjidId && !MASJID_ID) {
        console.log(`   💡 Tip: Set PROD_MASJID_ID=${user.masjidId} in .env for better testing`);
      }
      return authToken;
    } else {
...
async function runTests() {
  console.log(`\n${'#'.repeat(60)}`);
  console.log(`# LATENCY TEST - COA & LAPORAN KEUANGAN (PRODUCTION)`);
  ...
  if (!token) {
    ...
    console.log(`   Set PROD_JWT_TOKEN, or PROD_EMAIL and PROD_PASSWORD in .env file.`);
    console.log(`   Some tests may fail without authentication.\n`);
  }
  
  const results = [];
  
  // Test 1: COA - Get All Accounts
  ...
  // Test 3: COA - Get Account by ID (need to get an ID first)
  let accountId = null;
  try {
  ...
  // Test 4: Laporan - Neraca (Posisi Keuangan)
  ...
  // Test 5: Laporan - Penghasilan Komprehensif
  ...
  // Test 6: Laporan - Perubahan Aset Neto
  results.push(await testEndpoint(
  ...
  // Summary
  console.log(`\n\n${'#'.repeat(60)}`);
  console.log(`# SUMMARY (PRODUCTION)`);
  console.log(`#${'#'.repeat(59)}`);
  ...
  if (successfulResults.length > 0) {
    ...
    successfulResults.forEach(result => {
      if (result.avg > 2000) {
        console.log(`  ❌ ${result.name}: ${formatTime(result.avg)} - VERY SLOW (>2s)`);
      ...
      } else if (result.avg < 300) {
        console.log(`  ✅ ${result.name}: ${formatTime(result.avg)} - GOOD (<300ms)`);
      } else {
        console.log(`  ⚠️  ${result.name}: ${formatTime(result.avg)} - ACCEPTABLE (300-500ms)`);
      }
    });
  }
